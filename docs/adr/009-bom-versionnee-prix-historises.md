...
1. (si chantier fourni) **Grille rattachée au chantier** → `grille_chantier` *(migration 0017)*
2. **Prix de référence** (`fournisseur_id IS NULL`) → `reference` — **prix retenu dès qu'il est renseigné** *(réordonné par 0067)*
3. **Grille du fournisseur préféré** (sans chantier) → `grille_prefere` *(0016)*
4. **Prix du fournisseur préféré** (`prix_articles`) → `prefere`
5. **Grille la moins chère**, tous fournisseurs actifs → `grille_mini` *(0016)*
...
> était évalué *avant* les prix fournisseurs non-préférés. La migration **0061** l'avait basculé en
