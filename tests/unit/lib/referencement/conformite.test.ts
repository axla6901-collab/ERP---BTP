...
describe('statutDocument', () => {
  ...
  it('priorise le statut de workflow refusé', () => {
    const doc: DocumentLite = {
      natureDocumentId: 'kbis',
      statut: 'refuse',
      dateFinValidite: '2027-01-01',
    };
...
describe('evaluerConformiteTier', () => {
  ...
  it('classe "à jour" un tier sans corps d\'état (aucune obligation)', () => {
    const res = evaluerConformiteTier(
      { natureTiers: 'artisan', corpsEtatIds: [] },
      matrice,
      naturesById,
      new Map(),
      AUJ,
  ...
  it('classe "à relancer" si un document est manquant', () => {
    ...
    const res = evaluerConformiteTier(
      { natureTiers: 'artisan', corpsEtatIds: ['go'] },
      matrice,
      naturesById,
      docs,
      AUJ,
