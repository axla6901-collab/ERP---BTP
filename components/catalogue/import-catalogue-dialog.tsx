...
export const ImportCatalogueDialog = forwardRef<ImportCatalogueDialogHandle, Props>(
  function ImportCatalogueDialog({ fournisseurId, fournisseurNom, hideTrigger = false }, ref) {
    ...
    const [libelleGrille, setLibelleGrille] = useState(`Tarif ${fournisseurNom} ${anneeCourante}`);
    ...
    async function onFileChange(file: File) {
      ...
      try {
        ...
        if (res.data.suggestion) {
          ...
          const apercuRes = await previewImportCatalogue(base64, file.name, res.data.suggestion);
        ...
        {importActif && (
          <div className="min-w-0 space-y-3 rounded-md border bg-muted/10 p-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-sm font-medium">Importer un catalogue fournisseur</span>
            ...
            {analyse && mapping && feuilleCourante && (
              <div className="min-w-0 space-y-3 rounded-md border bg-background p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Les articles déjà connus (même code) ne sont pas recréés, mais leur prix est
                    ajouté à la grille. Familles et unités absentes sont créées automatiquement.
                  </p>
                </div>
                        ...
                        <tbody>
                          {preview.lignes.slice(0, 200).map((l) => (
                            ...
                            >
                              ...
                              <td className="px-2 py-1 text-right tabular-nums">{l.prix ?? '—'}</td>
                              <td className="px-2 py-1">{l.unite ?? '—'}</td>
                              <td className="px-2 py-1">
                                {l.erreurs.length > 0 ? (
                                  <span className="text-destructive">{l.erreurs.join('; ')}</span>
                                ) : l.doublon ? (
